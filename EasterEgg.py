class EasterEgg:
    ...
    def startCountdownEasterEgg(self):
        EXIT_TIMER = 3  # 프로그램 종료까지의 시간
        COUNTDOWN_STEP = 1  # 카운트다운
        return self.EASTER_EGG, EXIT_TIMER, COUNTDOWN_STEP

    def startStudentIdEasterEgg(self, expression):
        return self.EASTER_EGG + "\n" + "{}은 ".format(expression) + self.STUDENT_ID_EASTER_EGG_EXPRESSIONS.get(expression) + "의 학번 입니다."


    def isCountdownEasterEgg(self, expression):
        if expression == self.COUNTDOWN_EASTER_EGG_EXPRESSION:
            return True

    def isStudentIdEasterEgg(self, expression):
